...
async function render(data){
  ...
  function syncOverviewHeight(){const split=$('.split');if(innerWidth<=900){split.style.height='';return}const left=$('.leaderboard'),head=left.querySelector('.section-head'),ranking=$('#leaderboard'),leftStyle=getComputedStyle(left),headStyle=getComputedStyle(head),chrome=parseFloat(leftStyle.paddingTop)+parseFloat(leftStyle.paddingBottom)+parseFloat(leftStyle.borderTopWidth)+parseFloat(leftStyle.borderBottomWidth),height=Math.ceil(chrome+head.offsetHeight+parseFloat(headStyle.marginBottom||0)+ranking.scrollHeight);split.style.height=`${height}px`}
